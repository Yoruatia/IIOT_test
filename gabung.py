...
Halaman:
    1. LoginPage          — autentikasi (user: pi, pass: 1234)
...
class DobotIntegratedApp(tk.Tk):
    ...
    def show_frame(self, page_name):
        """Menampilkan halaman berdasarkan nama kelasnya"""
    ...
    def emergency_stop(self, event=None):
        """Menghentikan paksa proses subprocess yang sedang berjalan"""
    ...
    def get_script_path(self, script_name):
        """Helper untuk mencari lokasi script python target"""
    ...
    def run_script_blocking(self, script_name, args=None):
        """Fungsi pemanggil script eksternal lewat subprocess"""
        if args is None:
            args = []
...
# ============================================================
# PAGE 3: DIRECT CONTROL — 16 Tombol Grid Matriks 4×4
# ============================================================
class DirectControlPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Parameter simpanan koordinat terpilih
        ...
        # --- Grid 4x4 (Matriks Box Sisi 20 mm sesuai utility2) ---
    ...
    def _execute_selected(self):
        ...
        # Kunci seluruh UI selama proses pergerakan robot berlangsung
        for btn in self.all_buttons:
    ...
    def _run_thread(self, col, row):
        # Memanggil main.py dengan parsing argumen koordinat matriks
    ...
    def _unlock_buttons(self):
        """Mengembalikan status interaksi tombol setelah eksekusi selesai"""
        for btn in self.all_buttons:
            if btn == self.btn_back:
                btn.configure(state=tk.NORMAL, bg="#95a5a6")
            elif btn == self.btn_execute:
                btn.configure(state=tk.DISABLED)  # Kembali non-aktif sampai ada pilihan baru
            else:
...
class SmartSortPage(tk.Frame):
    ...
    def _browse_file(self):
        ...
        if path:
            ...
            self.lbl_info.configure(text=f"Path: {path}", fg="#95a5a6")
